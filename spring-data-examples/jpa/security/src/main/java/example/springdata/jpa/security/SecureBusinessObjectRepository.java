 ...
 *
 * The principal is the currently logged in user. However, you retrieve it through the security context
 * which is bound to the current thread and as such it's also bound to the current request and its session.
 *
 * SecurityContextHolder.getContext() internally obtains the current SecurityContext implementation
 * through a ThreadLocal variable. Because a request is bound to a single thread this will get you the
 * context of the current request.
 *
 * To simplify you could say that the security context is in the session and contains user/principal and
 * roles/authorities.
 */
